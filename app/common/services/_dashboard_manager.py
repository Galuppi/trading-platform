...
class DashboardManager:
    ...
    def _build_html(
        ...
        if mode == MODE_BACKTEST:
            ...
            weekly_performance_html = self._build_backtest_weekly(backtester_config, summary_writer)
        else:
            balances_html, weekly_performance_html = self._build_live_balances(state_manager)
        ...
        return self.HTML_TEMPLATE.format(
            ...
            weekly_performance_html=weekly_performance_html,
            timestamp=timestamp,
            utc_time=utc_time,
        )
    ...
    def _build_backtest_weekly(self, backtester_config, summary_writer):
        try:
            initial_deposit = float(backtester_config.backtest_deposit)
            begin_week_str = f"{initial_deposit:.2f}"
            profit_week = 0.0
            profit_class = "profit-neg" if profit_week < 0 else "profit-pos"
            profit_week_html = f"<span class='{profit_class}'>{profit_week:+.2f}</span>"
            return f"""
            ...
            return "<div class='warning'>Weekly data unavailable</div>"
    ...
    def _build_live_balances(self, state_manager):
        try:
            ...
            weekly_performance_html = f"""
            <div class="item"><span class="label">Begin balance week:</span> {begin_week_str}</div>
            <div class="item"><span class="label">Profit total week:</span> {profit_week_html}</div>
            """

            return balances_html, weekly_performance_html

        except Exception as exc:
            error = f"<div class='warning'>[ERROR] Live balance: {exc}</div>"
            return error, ""
