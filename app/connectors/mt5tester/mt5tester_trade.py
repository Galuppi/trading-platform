...
class Mt5testerTrade(Trade):
    def __init__(self, symbol, calculator: Calculator,
        ...
        self.result_file_path = (
            Path(RESULT_PATH) /
            f"results_{datetime.now().strftime('%Y%m%d%H%M')}.csv"
        )
    ...
    def open_position(self, order: OrderRequest) -> OrderResult:
        if not order.price or order.price <= 0:
            ...
            if market_price is None or market_price <= 0:
                return OrderResult(
                    ...
                    order_id="",
                    retcode=10001,
                    comment="Rejected: No historical data available for price",
                    deal=None,
                    request={}
                )
        ...
        order_result = OrderResult(
            ...
            order_id=trade_id,
            retcode=-1,
            comment="Simulated order",
            deal=PlatformTime.now().isoformat(),
            request={"entry_price": order.price}
        )
    ...
    def close_position(self, trade: TradeRecord) -> TradeResult:
        if trade.exit_price is None:
            ...
            if price is None or price <= 0:
                return TradeResult(
                    ...
                    profit=0.0,
                    closed=False,
                    comment="Rejected: No market price at close time"
                )
        ...
        if self.summary_writer:
            try:
                self.summary_writer.update_total_profit(
                    trade.profit,
                    PlatformTime.now()
                )
    ...
    def _log_close(self, trade: TradeRecord):
        if trade.exit_price is None:
            return
        ...
        with open(self.result_file_path, mode="a", newline="") as f:
            ...
            writer.writerow([
                PlatformTime.to_mt_time_format(trade.timestamp),
                trade.strategy.capitalize(),
                ...
                trade.stop_loss,
                trade.take_profit,
                PlatformTime.to_mt_time_format(trade.exit_time),
                trade.exit_price,
                trade.commission,
                0.0,
