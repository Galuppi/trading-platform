'''"""This module defines classes related to Mt5Account."""'''
import MetaTrader5 as mt5
import logging
from typing import List, Optional
from app.base.base_account import Account
from app.common.models.model_trade import OrderRequest, TradeRecord
from app.common.config.constants import TRADE_DIRECTION_BUY, TRADE_STATUS_CLOSED, DATETIME_FORMAT, TIME_REFERENCE_SYMBOL, RETRY_COUNT, RETRY_DELAY_SECONDS
...
class Mt5Account(Account):
    ...
    def get_margin_required(self: Any, order: OrderRequest) -> float:
        ...
        result = mt5.order_calc_margin(mt5.ORDER_TYPE_BUY if order.direction == TRADE_DIRECTION_BUY else mt5.ORDER_TYPE_SELL, order.symbol, order.lot_size, price)
    ...
    def has_sufficient_margin(self: Any, order: OrderRequest) -> bool:
        try:
            ...
            logger.warning(f'Margin check failed for {order.symbol} (lot size {order.lot_size}): {e}')
            return False
    ...
    def get_closed_tickets(self: Any, lookback_hours: int=24) -> List[TradeRecord]:
        ...
        for d in deals:
            deal = d._asdict()
            if not deal.get('symbol') or float(deal.get('volume', 0)) <= 0 or deal.get('entry') != mt5.DEAL_ENTRY_OUT:
                continue
            if deal.get('entry') != mt5.DEAL_ENTRY_OUT:
                continue
            trade = TradeRecord(id=str(deal['position_id']), symbol=deal['symbol'], lot_size=float(deal['volume']), type='buy' if deal['type'] in (mt5.DEAL_TYPE_BUY, mt5.ORDER_TYPE_BUY) else 'sell', ticket=str(deal['position_id']), status=TRADE_STATUS_CLOSED, timestamp=PlatformTime.datetime_str(), strategy='external', entry_price=None, exit_price=float(deal.get('price', 0.0)), exit_time=PlatformTime.from_timestamp(deal['time']).strftime(DATETIME_FORMAT), stop_loss=None, stop_loss_points=None, take_profit=None, commission=float(deal.get('commission', 0.0)), comment=str(deal.get('comment', '')), profit=float(deal.get('profit', 0.0)), slippage_entry=None)
    ...
    def _safe_account_info(self: Any) -> Any:
