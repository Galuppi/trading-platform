...
class BreakOutStrategy(Strategy):
    ...
    def set_open_price(self):
        ...
        for asset in self.assets:
            symbol = asset.symbol
            if symbol not in self.open_price_by_symbol:
                self.open_price_by_symbol[symbol] = None
            current_open = self.open_price_by_symbol[symbol]
            if current_open is not None:
                continue
            try:
                open_price = self.symbol.get_open_price(symbol)
                if open_price is not None:
                    self.open_price_by_symbol[symbol] = open_price
            except Exception as e:
                logger.warning(
