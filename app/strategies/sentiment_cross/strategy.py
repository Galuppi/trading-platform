...
VIX_PAUSE_THRESHOLD = 1.80


class SentimentCrossStrategy(Strategy):
    ...
    def _is_vix_paused(self, signal: Optional[SentimentSignal]) -> bool:
        if not signal or signal.vix is None:
            return False
        if signal.vix > VIX_PAUSE_THRESHOLD:
            logger.warning(f"VIX pause active ({signal.vix}) > {VIX_PAUSE_THRESHOLD}")
            return True
        return False
    ...
    def is_exit_signal(self, trade: TradeRecord, asset_config: AssetConfig) -> bool:
        ...
        if CLOSE_TRADES_ON_VIX_PAUSE and self._is_vix_paused(signal):
