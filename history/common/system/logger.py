'''"""This module provides functions including setup_logger."""'''
import os
import logging
...
def setup_logger(log_path: Path, mode: str) -> logging.Logger:
    ...
    logging.basicConfig(level=log_level, format='%(asctime)s [%(levelname)s] [%(name)s] %(message)s', handlers=[logging.FileHandler(log_file, encoding='utf-8'), logging.StreamHandler()])
