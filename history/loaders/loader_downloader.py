'''"""This module provides functions including load_downloader_config, discover_downloaders, downloader_registry_class."""'''
import importlib
import yaml
import logging
...
def load_downloader_config(config_path: Path) -> DownloaderConfig:
    ...
    return DownloaderConfig(name=raw_config.get('name', config_path.parent.name), enabled=raw_config.get('enabled', True), assets=assets)

def discover_downloaders() -> Any:
    for item in DOWNLOADER_PATH.iterdir():
        if item.is_dir() and (not item.name.startswith('__')):
            ...
            if downloader_file.exists() and config_file.exists():
                ...
                logger.warning(f"Skipping '{item.name}': downloader.py or config.yaml not found")

def downloader_registry_class(module_path: str, class_name: str) -> Any:
    module = importlib.import_module(module_path)
    if not hasattr(module, class_name):
        raise ImportError(f"Class '{class_name}' not found in '{module_path}'")
    return getattr(module, class_name)

def downloader_registry(connector: Any, history: Any) -> list:
    downloaders = []
    for item, _, config_file in discover_downloaders():
        try:
            ...
            class_name = ''.join((part.capitalize() for part in item.name.split('_'))) + 'Downloader'
